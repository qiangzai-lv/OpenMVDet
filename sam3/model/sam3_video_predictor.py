...
from sam3.device_utils import get_device_type, get_device_module, get_default_device, device_count, current_device, set_device, memory_allocated, memory_reserved, max_memory_allocated, max_memory_reserved, get_dist_backend, to_device
...
class Sam3VideoPredictor:
    ...
    def _get_session_stats(self):
        ...
        session_stats_str = (
            ...
            f"{memory_allocated() // 1024**2} MiB used and "
            f"{memory_reserved() // 1024**2} MiB reserved"
            f" (max over time: {max_memory_allocated() // 1024**2} MiB used "
            f"and {max_memory_reserved() // 1024**2} MiB reserved)"
        )
    ...
    def _get_torch_and_gpu_properties(self):
        ...
        if dev_mod is not None and hasattr(dev_mod, 'get_device_properties'):
            props = dev_mod.get_device_properties(current_device())
            arch_info = getattr(dev_mod, 'get_arch_list', lambda: 'N/A')()
            torch_and_gpu_str = (
                f"torch: {torch.__version__} with {dev_type.upper()} arch {arch_info}, "
                f"GPU device: {props}"
            )
        else:
            torch_and_gpu_str = f"torch: {torch.__version__} with device type: {dev_type}"
...
class Sam3VideoPredictorMultiGPU(Sam3VideoPredictor):
    def __init__(self, *model_args, gpus_to_use=None, **model_kwargs):
        ...
        self.device = torch.device(f"{get_device_type()}:{self.gpus_to_use[self.rank]}")
    ...
    def _start_nccl_process_group(self):
        ...
        tensor = torch.ones(1024, 1024).to(get_default_device())
