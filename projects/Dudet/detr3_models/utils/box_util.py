...
from projects.Dudet.detr3_models.utils.misc import to_list_1d, to_list_3d

# try:
from projects.Dudet.detr3_models.utils.box_intersection import box_intersection
# except ImportError:
#     print(
...
def get_3d_box_batch_depth_tensor(box_size, angle, center):
    """ 生成符合 MMDetection3D Depth 坐标系的 3D 包围框角点
    
    Args:
        box_size: 尺寸张量 (..., 3) [l, h, w] 对应 X/Y/Z 轴
        angle: 绕Z轴旋转角度（弧度）
        center: 中心坐标 (..., 3) [x, y, z] (Depth坐标系)
    
    Returns:
        corners_3d: 形状 (..., 8, 3)，可直接用于 generalized_box3d_iou_tensor
    """
    assert isinstance(box_size, torch.Tensor)
    assert isinstance(angle, torch.Tensor)
    assert isinstance(center, torch.Tensor)
    ...
    reshape_final = False
    if angle.ndim == 2:
        assert box_size.ndim == 3 and center.ndim == 3
        bsize, nprop = box_size.shape[0], box_size.shape[1]
    ...
    h = torch.unsqueeze(box_size[..., 1], -1)
    w = torch.unsqueeze(box_size[..., 2], -1)

    corners_3d = torch.zeros(
        (*angle.shape, 8, 3), 
        device=box_size.device, 
        dtype=torch.float32
    )
    ...
    corners_3d = torch.matmul(corners_3d, R.permute(0, 2, 1))
...
def get_3d_box_batch_tensor_ori_coda(box_size, angle, center):
    assert isinstance(box_size, torch.Tensor)
    assert isinstance(angle, torch.Tensor)
    assert isinstance(center, torch.Tensor)
