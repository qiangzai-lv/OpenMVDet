...
from OpenMVDet.detr3_models.helpers import (ACTIVATION_DICT, NORM_DICT, WEIGHT_INIT_DICT,
                                        get_clones)
from functools import partial
from OpenMVDet.detr3_models.helpers import GenericMLP
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if self.norm is not None:
            ...
            if self.return_intermediate:
                intermediate.pop()
                intermediate.append(output.permute(1, 2, 0))
        ...
        if self.return_intermediate:
            ...
            return intermediate, attns

        return output, attns

class TransformerDecoder_Multilevel(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm_fn_name="ln",
        ...
        self.task_head = self.mlp_func(output_dim=4)
        ######### idea 2 #########
    ...
    def forward(self, tgt, memory,
        ...
        for idx_layer, layer in enumerate(self.layers):
            if if_task_query:
                task_query = output[-1:]  # [1, batch_size, feat_dim] ?
                
                task_query = task_query.permute(1, 2, 0)  #[bs, feat_dim, 1]
                task_weights = self.task_head(task_query)  
                task_weights = torch.softmax(task_weights, dim=-2)
                
                weights = task_weights.permute(1, 2, 0).unsqueeze(-1)
                
                stacked_memory = torch.stack(memory, dim=0)
                
                weighted_memory = (weights * stacked_memory).sum(dim=0)  # [H*W, batch_size, feat_dim]
                input_memory = weighted_memory

            else:
                idx_feat = idx_layer // 2
                input_memory = memory[idx_feat]
            ...
            if self.return_intermediate:
                if len(output) == 257:
                   output[0] = output[0] + 0*output[-1]
                output_intermediate = output[:-1] if len(output) == 257 else output
                intermediate.append(self.norm(output_intermediate).permute(1, 2, 0))
            if return_attn_weights:
                attns.append(attn)
        ...
        if self.norm is not None:
            if len(output) == 257:
                output[0] = output[0] + 0*output[-1]
            output = output[:-1] if len(output)==257 else output
            output = self.norm(output)
            if self.return_intermediate:
                intermediate.pop()
                intermediate.append(output.permute(1, 2, 0))
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=256,
        ...
        self.norm1_task = NORM_DICT[norm_fn_name](d_model)
        self.norm2_task = NORM_DICT[norm_fn_name](d_model)

        self.norm3 = NORM_DICT[norm_fn_name](d_model)
        self.norm3_task = NORM_DICT[norm_fn_name](d_model)
        self.dropout1 = nn.Dropout(dropout, inplace=False)
        self.dropout2 = nn.Dropout(dropout, inplace=False)
        self.dropout3 = nn.Dropout(dropout, inplace=False)
        self.dropout1_task = nn.Dropout(dropout, inplace=False)
        self.dropout2_task = nn.Dropout(dropout, inplace=False)
        self.dropout3_task = nn.Dropout(dropout, inplace=False)
        ...
        self.linear1_task = nn.Linear(d_model, dim_feedforward)
        self.dropout_task = nn.Dropout(dropout, inplace=False)
        self.linear2_task = nn.Linear(dim_feedforward, d_model)


        self.activation = ACTIVATION_DICT[activation]()
        self.activation_task = ACTIVATION_DICT[activation]()
        self.normalize_before = normalize_before
    ...
    def forward_pre(self, tgt, memory,
                    ...
                    return_attn_weights: Optional [bool] = False):
        if len(tgt) == 257:
            tgt_main, tgt_task = tgt[:-1], tgt[-1:] 
            tgt2_main, tgt2_task = self.norm1(tgt_main), self.norm1_task(tgt_task)
            tgt2 = torch.cat([tgt2_main, tgt2_task], dim=0)

            tgt2_main_with_pos = self.with_pos_embed(tgt2_main, query_pos)
            q = k = torch.cat([tgt2_main_with_pos,tgt2_task], dim=0)
            tgt2 = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask,
                                key_padding_mask=tgt_key_padding_mask)[0]
            tgt2_main, tgt2_task = tgt2[:-1], tgt2[-1:] 
            tgt = tgt + torch.cat([self.dropout1(tgt2_main), self.dropout1_task(tgt2_task)], dim=0)
            #---------------------------------------------------------#
            tgt_main, tgt_task = tgt[:-1], tgt[-1:] 
            tgt2_main, tgt2_task = self.norm2(tgt_main), self.norm2_task(tgt_task)

            tgt2_main_with_pos = self.with_pos_embed(tgt2_main, query_pos)
            query = torch.cat([tgt2_main_with_pos,tgt2_task],dim=0)
            tgt2, attn = self.multihead_attn(query=query,
                                    key=self.with_pos_embed(memory, pos),
                                    value=memory, attn_mask=memory_mask,
                                    key_padding_mask=memory_key_padding_mask)
            tgt2_main, tgt2_task = tgt2[:-1], tgt2[-1:] 
            tgt = tgt + torch.cat([self.dropout2(tgt2_main), self.dropout2_task(tgt2_task)], dim=0)
            #----------------------------------------------------------#
            tgt_main, tgt_task = tgt[:-1], tgt[-1:]
            tgt2_main, tgt2_task = self.norm3(tgt_main), self.norm3_task(tgt_task)

            tgt2_main_ffn = self.linear2(self.dropout(self.activation(self.linear1(tgt2_main))))
            tgt2_task_ffn = self.linear2_task(self.dropout_task(self.activation_task(self.linear1_task(tgt2_task))))

            tgt2 = torch.cat([self.dropout3(tgt2_main_ffn), self.dropout3_task(tgt2_task_ffn)], dim=0)
            tgt = tgt + tgt2
            if return_attn_weights:
                return tgt, attn
            return tgt, None
